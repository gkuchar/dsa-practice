    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(row, col, curr_layer, visited):
            ...
            if 'word' in curr_layer[curr_char]:
                result.append(curr_layer[curr_char]['word'])
                curr_layer[curr_char].pop('word')
        ...
        # T = O(rows * cols # 3^L), L = length of longest words
        # 3^L since each dfs call moves in 3 with each direction going L more cells at the most

        # S = O(s), s = sum of all chars in words.
        # s dominates recursive call stack and visted set